...
interface LotteryService {
    suspend fun getLottery(encryptCode: String): Flow<Result<LotteryEntity>>
}

class LotteryServiceImpl(private val lotteryApi: RetrofitApi) : LotteryService {

    override suspend fun getLottery(encryptCode: String): Flow<Result<LotteryEntity>> = flow {
        emit(Result.Loading)
        emit(
            try {
                val response = lotteryApi.getLottery(PostParam(encryptCode = encryptCode))
